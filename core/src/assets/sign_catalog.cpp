...
//
// model_id names the mesh a placement draws with. Until the per-designation
// sign meshes land (#414 PR B) several designations legitimately share the
// legacy silhouettes; find_by_identity() callers must tolerate a model id that
// props::model() does not know, because a catalogue can outrun the asset
// bundle in exactly that window.
