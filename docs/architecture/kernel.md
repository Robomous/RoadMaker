...
| `assets/` | `prop_library.hpp`, `prop_import.hpp`, `sign_catalog.hpp`, `sign_face.hpp` | The bundled prop and sign models (procedural, generated by `scripts/gen_prop_meshes.py`), the sign-face rasteriser, and glTF/GLB import of a user's own models ([ADR-0013](../decisions/0013-asset-import-gltf-and-images.md) — OBJ, USD read and FBX are refused by name; a textured model imports flat because `PropPart` carries no UVs). `props::model()` resolves the open project's imported models ahead of the bundled ones |
