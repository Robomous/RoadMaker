...
see [Edit Nodes](../reference/edit-nodes.md) for inserting and deleting them.
...
[T-junction](../reference/t-junction.md) page walks through the exact gesture and what the
generated connecting lanes look like.

Prefer a full intersection instead of a tee? Draw both road ends into the same
area and use the [Junction](../reference/junction.md) tool to connect them with generated
turning lanes.

## 3. Check it

Open the **Scene** panel to confirm both roads and the junction are present, and
glance at the [Diagnostics](../reference/diagnostics.md) panel — a clean network raises no
...
USD), see [Save & export](../reference/save-export.md).
