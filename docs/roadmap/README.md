...
[GW-1 … GW-6](golden_workflows/README.md): step-by-step scripts a human
executes in RoadMaker, each step
...
| P7 | Import & Export | Scene Export Preview + OpenDRIVE Export Preview tools; **world georeference settings** — a world projection from standard CRS descriptions (WKT / proj-string), a world origin (latitude/longitude), workspace extents, and center/fit-to-selection in that frame; imports reproject into the world frame and exports record `<header><geoReference>` (OpenDRIVE §8.5); GIS vector/raster import — GeoJSON, ESRI Shapefile, GeoTIFF and world-filed imagery, reprojected from a bounded CRS family (WGS84 geographic, UTM, Transverse Mercator, Web Mercator) computed in closed form, with everything outside it refused by name ([ADR-0010](../decisions/0010-gis-ingest-bounded-crs.md): no GDAL, no PROJ); lidar import — ASPRS LAS 1.0–1.4 and LAZ read in RoadMaker's own code through that same CRS family, decimated for display and ground-fitted into the height field ([ADR-0011](../decisions/0011-lidar-ingest-in-house-las.md): no PDAL, because it hard-requires the PROJ that ADR-0010 declined); OSM road-network extraction with diagnostics-first fitting — `.osm` XML read in-house through the same CRS family, ways classified onto the four road classes and joined on OSM's own shared-node graph, with every dropped or approximated element named ([ADR-0012](../decisions/0012-osm-ingest-xml-in-house.md): no new dependency at all; `.osm.pbf` refused because it needs the zlib that #484 owns; mapping table in [osm_mapping.md](../domain/osm_mapping.md)), plus the scale/perf harness measuring a 1k-road network and a 50 km² import in CI (supersedes #54 scale targets) | GW-2 (previews) |
| P8 | Scenarios | Internal scenario model exporting **OpenSCENARIO 1.x XML** first (validation-friendly, esmini-compatible), **kernel-side and `EditStack`-driven so the golden workflow keeps headless evidence** ([ADR-0014](../decisions/0014-scenario-model-kernel-side-osc-1x.md): namespace `roadmaker::osc`, a deterministic `write_xosc` defaulting to revision 1.2, only `odr_id` crossing the file boundary, and esmini as the validator), with **OpenSCENARIO 2.x as an explicit later sprint** — an export-only concrete-scenario subset at v0.1.0, no OSC2 import or parser dependency; Map ↔ Scenario mode; actor placement; lane-anchored routes; storyboard/condition logic editor; esmini preview hooks. [GW-6](golden_workflows/gw6_scenarios.md) was drafted at P8 planning, before the first sprint, so #245–#248 have an acceptance target | GW-6 |
