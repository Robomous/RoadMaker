...
TEST(HelpRegistry, EveryToolIdHasExactlyOnePage) {
  ...
  // LaneCarve is the last enumerator (tools/tool.hpp); every value at or below
  // it must have a row, so a new ToolId added without one fails here.
  for (int i = 0; i <= static_cast<int>(ToolId::LaneCarve); ++i) {
