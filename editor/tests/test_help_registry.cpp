...
TEST(HelpRegistry, EveryToolIdHasExactlyOnePage) {
  ...
  // TerrainBrush is the last enumerator (tools/tool.hpp); every value at or
  // below it must have a row, so a new ToolId added without one fails here. Keep
  // this bound on the REAL last value: it used to stop at Corner (with a comment
  ...
  // at Signal, leaving Sign (p4-s9, #230) unchecked (p1-s6, #368); then at Sign,
  // leaving Surface (p5-s1, #231) unchecked until the Terrain Brush (p5-s4, #234)
  // pushed the bound to the real end.
  for (int i = 0; i <= static_cast<int>(ToolId::TerrainBrush); ++i) {
